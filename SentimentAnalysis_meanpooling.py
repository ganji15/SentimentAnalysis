...
def train():
    ...
    for epoch in xrange(num_epoch):
        ...
        if best_acc < acc:
            best_acc = acc
            cPickle.dump((input_var, mask_var, network), open(meanpooling_path, 'w'))
            print 'save lstm to %s, best valid accuracy: %.2f%%\n'%(meanpooling_path, best_acc * 100)
...
def test():
    ...
    index = T.lscalar('index')
    target_var = T.ivector('target')
    input_var, mask_var, network = cPickle.load(open(meanpooling_path, 'r'))
