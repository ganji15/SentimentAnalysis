...
def build_model(max_seq_length, input_var = None, mask_var = None):
    ...
    l_meanpooling = MeanPoolingLayer(l_lstm, mask_input = mask_var)
...
def train():
    ...
    for epoch in xrange(num_epoch):
        ...
        if best_acc < acc:
            best_acc = acc
            cPickle.dump((input_var, mask_var, network), open(meanpooling_lstm_path, 'w'))
            print 'save mlstm to %s, best valid accuracy: %.2f%%\n'%(meanpooling_lstm_path, best_acc * 100)
...
def test():
    ...
    index = T.lscalar('index')
    target_var = T.ivector('target')
    input_var, mask_var, network = cPickle.load(open(meanpooling_lstm_path, 'r'))
